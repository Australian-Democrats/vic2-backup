...
(function () {
  ...
  // Press releases upload a PDF through fields.file, which takes no accept
  // filter of its own — so "the release" can arrive as a .docx renamed .pdf, or
  // as a Pages export, and the reader gets a Download button that hands them a
  // file nothing opens. Same defence, same layer: sniff it here, where the
  // bytes actually are, rather than at the deploy gate — which holds back the
  // whole site and must never fail on an editor's upload.
  var DOC_EXT = /\.pdf$/i;
  ...
  function sniff(bytes) {
    ...
    // Word (.docx) and Pages files are ZIP containers. Naming one .pdf is the
    // most likely way a press release arrives in the wrong format, so it gets
    // its own answer rather than "unrecognised".
    if (bytes.length >= 4 && ascii(0, 2) === 'PK' && bytes[2] === 0x03 && bytes[3] === 0x04) return 'zip';
  ...
  var EXT_OK = {
    png: ['png'], jpg: ['jpg'], jpeg: ['jpg'], webp: ['webp'], gif: ['gif'], svg: ['svg'], ico: ['ico'],
    pdf: ['pdf'],
  };
  ...
  function checkFile(path, b64contents) {
    ...
    if (!m || (!IMG_EXT.test(path) && !DOC_EXT.test(path))) return null;
  ...
  function explain(problems) {
    var lines = problems.map(function (p) {
      ...
      if (p.kind === 'heic') {
        ...
        advice = 'The Download button on the release hands readers this exact file, so it has to be a ' +
          'real PDF. In Word or Pages choose File → Export/Save As → PDF (not "rename to .pdf"), ' +
    ...
    var noun = problems.some(function (p) { return p.ext === 'pdf'; }) ? 'file' : 'image';
    return 'This ' + noun + ' can’t be published:\n\n' + lines.join('\n\n') +
  ...
  function showBlockedMessage(problems) {
    ...
    try {
      ...
      h.textContent = problems.some(function (p) { return p.ext === 'pdf'; })
        ? 'Upload blocked'
